...
void SDA_IN(void)
{
    GPIOB->MODER &= ~(3 << (7 * 2)); GPIOB->MODER |= 0 << 7 * 2;  //PB7输入模式
}
void SDA_OUT(void)
{
    GPIOB->MODER &= ~(3 << (7 * 2)); GPIOB->MODER |= 1 << 7 * 2;  //PB7输出模式
}
...
返回   成功 为1 
 		失败为0
*******************************************************************************/ 
u8 IICwrite_Bits(u8 dev,u8 reg,u8 bitStart,u8 length,u8 data)
{
...
返回   成功 为1 
 		失败为0
*******************************************************************************/ 
u8 IICwrite_Bit(u8 dev, u8 reg, u8 bitNum, u8 data){
    u8 b;
